...
const Basics = () => {
        ...
        for (const uid of uids) {
          try {
            const response = await axios.put(
              `http://localhost:5000/api/users/name/${uid}`
            );
    ...
    const fetchPushedUids = async () => {
      try {
        const res = await axios.get(
          "http://localhost:5000/api/agora/pushed-uids",
          {
            headers: { Authorization: `Bearer ${user.token}` },
          }
        );
  ...
  useEffect(() => {
    ...
    const fetchPromotedUid = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5000/api/agora/promote-uid/${linkId}`,
          {
            headers: { Authorization: `Bearer ${user.token}` },
          }
        );
  ...
  const handlePushRequest = async () => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/api/agora/push-uid",
        { email: user.email },
        {
          headers: { Authorization: `Bearer ${user.token}` },
        }
  ...
  const handleResetOwnRequest = async (uidToRemove) => {
    ...
    try {
      await axios.delete(
        `http://localhost:5000/api/agora/unpush-uid/${uidToRemove || email}`,
        {
          headers: { Authorization: `Bearer ${user.token}` },
        }
      );
  ...
  return (
    <div className="flex overflow-hidden">
      ...
      >
        ...
        {isConnected && (
          <div>
            <div className="flex flex-col -mt-4 p-3 lg:flex-row gap-4">
