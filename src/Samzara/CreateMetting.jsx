...
const CreateMeeting = () => {
  ...
  const fetchUpcomingMeetings = async () => {
    if (!user) return;
    try {
      const response = await axios.get("http://localhost:5000/api/agora/Upcomeing-rooms", { headers: { Authorization: `Bearer ${user.token}` } });
  ...
  return (
    <div className="w-full min-h-screen  p-4 sm:p-6 md:p-6 md:rounded-2xl">
      <div className="bg-white w-full rounded-2xl -mt-2.5 shadow-sm p-4 sm:p-6 mb-6">
