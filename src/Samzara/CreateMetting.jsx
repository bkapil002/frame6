...
const CreateMeeting = () => {
  ...
  const fetchUpcomingMeetings = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:5000/api/agora/Upcomeing-rooms",
  ...
  useEffect(
    () => setSelectedHour(parseTime(selectedSlot).hours),
    [selectedSlot]
  );
            ...
            <div className="flex flex-wrap justify-between items-center mb-4 gap-2">
              <div className="flex gap-2 items-center">
                ...
                <span className="font-medium md:text-sm text-xs text-[#3C3C3C]">
                  {daysArr[0].format("D MMM ")} –{" "}
                  {daysArr[daysArr.length - 1].format("D MMM , YYYY")}
                </span>
              </div>
